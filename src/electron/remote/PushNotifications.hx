...
@:enum abstract PushNotificationsEvent<T:(haxe.Constraints.Function)>(js.node.events.EventEmitter.Event<T>) to js.node.events.EventEmitter.Event<T> {
	/**
	...
	**/
	var received_apns_notification : electron.remote.PushNotificationsEvent<Void -> Void> = "received-apns-notification";
}
