...
@:jsRequire("electron", "UtilityProcess") extern class UtilityProcess extends js.node.events.EventEmitter<electron.UtilityProcess> {
	...
	**/
	...
	var stdio : haxe.extern.EitherType<Array<String>, String>; /**
		Name of the process that will appear in `name` property of `child-process-gone` event of `app`. Default is `node.mojom.NodeService`.
	**/
	@:optional
...
@:enum abstract UtilityProcessEvent<T:(haxe.Constraints.Function)>(js.node.events.EventEmitter.Event<T>) to js.node.events.EventEmitter.Event<T> {
	/**
