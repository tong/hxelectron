package electron;
@:jsRequire("electron", "app") @:require("electron") extern class App extends js.node.events.EventEmitter<electron.App> {
	/**
	...
	**/
	...
	/**
		Adds tasks to the Tasks category of the JumpList on Windows. tasks is an array of Task objects in the following format: Task Object: Note: If you'd like to customize the Jump List even more use app.setJumpList(categories) instead.
	**/
	static function setUserTasks(tasks:Array<Task>):Bool;
	static function getJumpListSettings():Dynamic;
	/**
	...
	**/
	static function getLoginItemSettings():Dynamic;
	/**
