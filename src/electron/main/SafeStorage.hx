...
@:jsRequire("electron", "safeStorage") extern class SafeStorage extends js.node.events.EventEmitter<electron.main.SafeStorage> {
	...
	**/
	...
	/**
		...
		* `gnome_any` - When the desktop environment is `X-Cinnamon`, `Deepin`, `GNOME`, `Pantheon`, `XFCE`, `UKUI`, `unity` or if the following command line flag is provided `--password-store="gnome"`. When this value is present the application will first try to use `libsecret` backend and if it fails will attempt to use `libgnome_keyring`.
